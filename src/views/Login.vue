                ...
                <sui-card-content>
                    ...
                    <sui-card-description>                        
                        <sui-form method="post" v-on:submit.prevent="Login">
                            ...
                            <sui-button v-if="!loading" fluid positive content="Connexion" type="submit" />
                            <sui-button v-else loading fluid positive content="Loading" />
                            <router-link to="/signup" class="text-center d-block mt-3 text-white">S'inscrire</router-link>
                            <router-link to="/analyse" class="text-center d-block mt-3 text-white">Analyse</router-link>
                        </sui-form>
                    </sui-card-description>
                </sui-card-content>
                <Alert v-if="message" title="Hmm.." typeMessage="error" :message="message" />
            </sui-card>
        </div>
    </div>
</template>
...
export default {
        ...
        Login() {
            ...
            this.$axios
              ...
              .then(res => {
                ...
                res.data.user_permission === 1 ? localStorage.setItem("isAdmin", res.data.user_permission) : ''
                this.$store.commit('CONNECT_USER', res.data);
                this.loading = false;
    ...
    beforeMount () {
        localStorage.getItem('first_name') ? localStorage.removeItem('first_name') : ''
        localStorage.getItem('last_name') ? localStorage.removeItem('last_name') : ''
        localStorage.getItem('image') ? localStorage.removeItem('image') : ''
        localStorage.getItem('banner') ? localStorage.removeItem('banner') : ''
...
<style lang="scss">
    .form-card.ui.card, .ui.cards > .card {
